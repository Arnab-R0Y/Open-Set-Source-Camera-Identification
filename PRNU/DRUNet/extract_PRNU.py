...
# Add the project root (OSCII folder) to Python path
...
def noise_extract_drunet(im: np.ndarray, levels: int = 4, sigma: int = 5) -> np.ndarray:
    ...
    if 'color' in model_name:
        ...
        model_pool = r'X:\Projects\Open set source camera identification\Model Path\DRUNet_color'
    else:
        n_channels = 1              # 1 for grayscale image
        model_pool = r'X:\Projects\Open set source camera identification\Model Path\DRUNet_gray'
...
def extract_multiple_aligned(imgs: list, levels: int = 4, sigma: int = 5, processes: int = None,
    ...
    if processes is None or processes > 1:
        ...
        with Pool(processes=processes) as pool:
            for batch_idx0 in tqdm(np.arange(start=0, step=batch_size, stop=len(imgs)),
                                disable=tqdm_str == '',
                                desc=(tqdm_str + ' (processing batches)'), dynamic_ncols=True):
                # Step 1: inten_sat_compact
                nni = pool.map(inten_sat_compact, args_list[batch_idx0:batch_idx0 + batch_size])
                for ni in nni:
                    NN += ni
                del nni

                # Step 2: noise_extract_compact
...
def gt(l1: Union[list, np.ndarray], l2: Union[list, np.ndarray]) -> np.ndarray:
